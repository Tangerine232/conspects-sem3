...
\begin{proof}
    $f$ определена в $U_{\delta^0}(x^0)$. \\
    $\varphi_k$ непрерывна в точке $t^0$. Тогда $\exists\eta^0\colon \forall t\in U_{\eta^0}(t^0)$ выполняется
    \[
    |\varphi_k(t)-x_k^0|<\frac{\delta^0}{\sqrt d}.
    \]
    Отсюда
    \[
    \sqrt{\sum_{k=1}^d(\varphi_k(t)-x_k^0)^2}<\delta^0.
    \]
    Следовательно, $(\varphi_1(t),\dots,\varphi_d(t))\in U_{\delta^0}(x^0)$.

    Зададим приращения
    \[
    \Delta\varphi_k=\varphi_k(t^0+\Delta t)-\varphi_k(t^0),
    \]
    \[
    \Delta x_k=\Delta\varphi_k.
    \]
